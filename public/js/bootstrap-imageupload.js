...
(function($) {
    ...
    $.fn.imageupload.defaultOptions = {
        allowedFormats: ['jpg', 'jpeg', 'png', 'gif', 'zip', 'txt', 'doc', 'docx', 'pdf', 'ppt', 'pptx'],
        removeImageThumbnail: ['zip', 'txt', 'doc', 'docx', 'pdf', 'ppt', 'pptx'],
        maxWidth: 250,
        maxHeight: 250,
        maxFileSizeKb: 80000
    };
    ...
    function getImageThumbnailHtml(src) {
        return '<img src="' + src + '" alt="Image preview" class="thumbnail" style="width: ' + options.maxWidth + 'px; height: ' + options.maxHeight + 'px"><br><br>';
    }
    ...
    function isValidImageFile(file, callback) {
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            ...
            callback(false, 'File type is not allowed. Upload [ jpg, jpeg, png, gif, zip, txt, doc, docx, pdf, ppt, pptx ] ONLY');
        }
    }
        ...
        image.onload = function() {
            if (!timeout) {
                ...
                if ($.inArray(fileExtension, options.allowedFormats) > -1) {
                    ...
                    callback(false, 'File type is not allowed. Upload [ jpg, jpeg, png, gif ] ONLY');
                }
            }
        };
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
                    ...
                    var fileExtension = getFileExtension(file.name);
                    if ($.inArray(fileExtension, options.removeImageThumbnail) > -1) {
